	...
	public static function get_google_font_json() {
		...
		if (class_exists('WP_Font_Manager')) {
			...
			foreach ($wfm_fonts as $font) {
				...
				foreach ($font['enabled']['variants'] as $variant) {
					$temp_variant = str_replace('italic', '', $variant);

					if ($temp_variant == 'regular') {
						$temp_variant = 400;
					}

					if (!empty($temp_variant)) {
						$weights[] = $temp_variant;
					}
				...
				$all_fonts[$font['all']['family']] = $weights;
			}

			$json = json_encode($all_fonts);
		}
